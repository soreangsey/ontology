    ...
    def __init__(self, parent, frame_):
        ...
        self.m_staticText4 = wx.StaticText(self.m_panel3, wx.ID_ANY,
                                           u'''This tool is made to facilitate agile methods adopting by answering to most common concerns you may have when adopting a practice. 
                                               ...
                                               * Hover your mouse on the phrase for the tooltip''', wx.DefaultPosition,
                                           wx.Size(-1, -1),
                                           wx.ALIGN_LEFT)
...
class InputPage2(wx.Panel):
    ...
    def __init__(self, parent, frame_):
        ...
        self.Centre(wx.BOTH)
        # Connect Events
        self.m_button1.Bind(wx.EVT_BUTTON, self.firstInputPage)
    ...
    def ShowResult(self, event):
        ...
        self.frame_.runQuery.createNewInstance()  # create new instance,
        # x = threading.Thread(target = self.frame_.runQuery.RunReasoner)#run reasoner and the query based on team
        # x.start()
        progressMax = 10
        dialog = wx.ProgressDialog("Please wait, the reasoner is being executed", "Time remaining", progressMax,
...
class InputPage1(wx.Panel):
    ...
    def __init__(self, parent, frame_):
        ...
        self.Centre(wx.BOTH)
        # Connect Events
        self.m_button0.Bind(wx.EVT_BUTTON, self.secondInputPage)
    ...
    def secondInputPage(self, event):
        ...
        # new_frame.Show()
        self.frame_.m_auinotebook1.SetSelection(3)
...
class ResultPage1(wx.Panel):
    ...
    def __init__(self, parent, frame_):
        ...
        self.m_scrolledWindow1 = wx.ScrolledWindow(self, wx.ID_ANY, wx.DefaultPosition, wx.Size(1000, -1),
                                                   wx.HSCROLL | wx.VSCROLL)
        self.m_scrolledWindow1.SetScrollRate(5, 5)
        bSizer7 = wx.BoxSizer(wx.VERTICAL)
        practice = "Daily meeting"
        self.questionList = ['The team\'s goal can achieve by adopting an agile practice',
                             'The agile value can be achieved by adopting a practice',
                             'The agile principle can be achieved by adopting a practice',
        ...
        self.m_bcomboBox1 = wx.ComboBox(self.m_scrolledWindow1, wx.ID_ANY, self.questionList[0], wx.DefaultPosition,
                                        wx.Size(1000, -1), style=wx.CB_READONLY, choices=self.questionList)
    ...
    def LoadResult(self, event, questionIndex=None):
        ...
        if questionIndex == 1 or questionIndex == 2 or questionIndex == 5 or questionIndex == 6 or questionIndex == 7 or questionIndex == 8:
            ...
            previous1 = ""
            previous2 = ""
            self.list.InsertColumn(0, column0[questionIndex], wx.LIST_FORMAT_LEFT, width=500)
            self.list.InsertColumn(1, column1[questionIndex], wx.LIST_FORMAT_LEFT, width=800)
            self.list.InsertColumn(2, column2[questionIndex], wx.LIST_FORMAT_LEFT, width=500)
            itemIndex = 0
            ...
            previous = ""
            itemIndex = 0
            self.list.InsertColumn(0, column0[questionIndex], wx.LIST_FORMAT_LEFT, width=800)
            self.list.InsertColumn(1, column1[questionIndex], wx.LIST_FORMAT_LEFT, width=1000)
...
class ResultPage2(wx.Panel):
    ...
    def __init__(self, parent, frame_):
        ...
        self.m_scrolledWindow1 = wx.ScrolledWindow(self, wx.ID_ANY, wx.DefaultPosition, wx.Size(1000, -1), 0)
        self.m_scrolledWindow1.SetScrollRate(5, 5)
        bSizer7 = wx.BoxSizer(wx.VERTICAL)

        self.questionList = ['The team\'s goal can achieve by adopting an agile practice',
                             'The practice(s) to adopt to achieve desired agile value',
                             'The practice(s) to adopt to achieve desired agile principle',
                             'The activity a team should perform as part of a practice',
                             'The problem a team may encounter while adopting a practice',
                             'The solution a team may use to solve the problem',
                             ...
                             'The artifact required for adopting a practice',
                             'The roles or responsibility distribution needed for a practice',
                             'The roles or responsibility distribution needed to solve problem',
                             'The requisites a team should prepare in order to successfully adopt a practice',
    ...
    def Copy(self, event):
        ...
        if questionIndex == 0 or questionIndex == 3 or questionIndex == 8 or questionIndex == 9 or questionIndex == 12 or questionIndex == 13:
            if event.GetX() < self.list.GetColumnWidth(0):
                text = objectEvent.GetItemText(objectEvent.GetFocusedItem(), col=0)
    ...
    def LoadResult(self, event, questionIndex=None):
        ...
        column1 = ['Achieves Goal', 'Has Sub-goal', 'Has Sub-goal', 'Composed of Activity', 'Encounters Problem', '',
                   'Harms Requisite', 'Helps Requisite', 'Requires Artifact', 'Requires Role', '', 'Requires Requisite',
                   'Has Lesson learned']
        column2 = ['', 'Achieved By Practice', 'Achieved by Practice', '', '', 'Required by Practice',
                   'Required by Practice', 'Required by Practice', 'For Activity', 'In responsible of Activity', '',
                   'In responsible of Activity']
        itemIndex = 0
        prefix0 = ['Practice:', 'Value:', 'Principle:', 'Practice:', '', '', 'Situation:', 'Situation:', 'Practice:',
                   'Practice:', '', 'Practice:', 'Practice:']
        prefix1 = ['Goal:', 'Goal:', 'Goal:', 'Activity:', '', '', 'Requisite:', 'Requisite:', 'Artifact:', 'Role:', '',
                   'Requisite:', 'Lesson:']
        prefix2 = ['', 'Practice:', 'Practice:', '', '', '', 'Practice:', 'Practice:', 'Activity:', 'Activity:']

        practiceName = [u"Practice:Daily_meeting", u"Practice:Short_iteration", u"Practice:Sprint_planning",
                        u"Practice:Sprint_review", u"Practice:Sprint_retrospective"]

        if questionIndex == 0:  # filter by team and agile practice
            self.list.InsertColumn(0, column0[questionIndex], wx.LIST_FORMAT_LEFT, width=500)
            self.list.InsertColumn(1, column1[questionIndex], wx.LIST_FORMAT_LEFT, width=1000)
            ...
            # print (self.frame_.valueList)
            # print (self.result[5][5])

            previous1 = ""
            ############################# we want to list Value, Goal, Practice
            self.list.InsertColumn(0, column0[questionIndex], wx.LIST_FORMAT_LEFT, width=500)
            self.list.InsertColumn(1, column1[questionIndex], wx.LIST_FORMAT_LEFT, width=800)
            self.list.InsertColumn(2, column2[questionIndex], wx.LIST_FORMAT_LEFT, width=500)
            itemIndex = 0

            for i, each_value in enumerate(self.result[5][5]):
                if each_value[0] in self.frame_.valueList:
                    if previous1 == each_value[0]:
                        itemIndex = self.list.InsertItem(itemIndex, '')
                    else:
                        itemIndex = self.list.InsertItem(itemIndex,
                                                         each_value[0].replace("_", " ")[len(prefix0[questionIndex]):])
                        previous1 = each_value[0]

                    self.list.SetItem(itemIndex, 1, each_value[1].replace("_", " ")[len(prefix1[questionIndex]):])

                    practiceList = ""
                    for index, practice in enumerate(practiceName):
                        for each_result in enumerate(self.result[index][0]):
                            # print (each_result)
                            if each_result[1] == each_value[
                                1]:  # each_principle [1] is goal, each_result[1] is goal too
                                practiceList += practiceName[index].replace("_", " ").replace("Practice:", "") + ","
                    self.list.SetItem(itemIndex, 2, practiceList[:-1])
                    itemIndex += 1

            ############################# If we want to list Value, Principle, Practice
        ##                self.list.InsertColumn(0, "Value",wx.LIST_FORMAT_LEFT, width = 500)
        ##                self.list.InsertColumn(1, "Contributed by Principle", wx.LIST_FORMAT_LEFT, width = 800)
        ##                self.list.InsertColumn(2, "Achieved by Practice, wx.LIST_FORMAT_LEFT, width = 500)
        ##                itemIndex = 0
        ##                for i, each_value in enumerate(self.result[5][10]):
        ##                        if each_value[0] in self.frame_.valueList:
        ##                            if previous1 == each_value[0]:
        ##                                itemIndex = self.list.InsertItem(itemIndex, '')
        ##                            else:
        ##                                itemIndex = self.list.InsertItem(itemIndex, each_value[0].replace("_"," ").replace("Value:"," "))
        ##                                previous1 = each_value[0]
        ##
        ##
        ##                            self.list.SetItem(itemIndex , 1, each_value[1].replace("_"," ").replace("Principle:"," "))
        ##                            print ("principle with value", each_value[1])
        ##                            practiceList = []
        ##
        ##                            for index, each_result in enumerate(self.result[5][2]):
        ##                                print (each_result)
        ##                                if each_result[0] == each_value[1]: #each_value [1] is principle, each_result[0] is principle too
        ##                                        if each_result[1].replace("_"," ").replace("Practice:","") not in practiceList:
        ##                                                practiceList.append(each_result[1].replace("_"," ").replace("Practice:",""))
        ##
        ##                            self.list.SetItem(itemIndex , 2, ','.join(practiceList))
        ##                            itemIndex += 1
            ...
            self.list.InsertColumn(2, column2[questionIndex], wx.LIST_FORMAT_LEFT, width=500)
            itemIndex = 0
            for i, each_principle in enumerate(self.result[5][4]):
                if each_principle[0] in self.frame_.principleList:
                    if previous1 == each_principle[0]:
                        itemIndex = self.list.InsertItem(itemIndex, '')
                    else:
                        itemIndex = self.list.InsertItem(itemIndex, each_principle[0].replace("_", " ")[
                                                                    len(prefix0[questionIndex]):])
                        previous1 = each_principle[0]

                    self.list.SetItem(itemIndex, 1, each_principle[1].replace("_", " ")[len(prefix1[questionIndex]):])

                    practiceList = ""
                    for index, practice in enumerate(practiceName):
                        for each_result in enumerate(self.result[index][0]):
                            # print (each_result)
                            if each_result[1] == each_principle[
                                1]:  # each_principle [1] is goal, each_result[1] is goal too
                                practiceList += practiceName[index].replace("_", " ").replace("Practice:", "") + ","
                    self.list.SetItem(itemIndex, 2, practiceList[:-1])
                    itemIndex += 1
            ...
            self.list.InsertColumn(0, "Practice", wx.LIST_FORMAT_LEFT, width=600)
            self.list.InsertColumn(1, "Activity", wx.LIST_FORMAT_LEFT, width=1200)
            ...
            previous1 = ""
            previous2 = ""
            self.list.InsertColumn(0, "Practice", wx.LIST_FORMAT_LEFT, width=500)
            self.list.InsertColumn(1, "Cause", wx.LIST_FORMAT_LEFT, width=300)
            self.list.InsertColumn(2, "Problem", wx.LIST_FORMAT_LEFT, width=1000)
            # print (self.frame_.resultByTeam)
            allTeamProblem = []
            ...
            self.list.InsertColumn(2, "Solution", wx.LIST_FORMAT_LEFT, width=600)
            ...
            for index, practice in enumerate(practiceName):
                if practice in self.frame_.practiceList:
                    ...
                    if len(self.result[index][questionIndex - 1]) > 0:
                        practice = practice.replace("_", " ")
                        practice = practice.replace("Practice:", "")
                    # print(self.result[index][questionIndex])
                    for i, each in enumerate(self.result[index][questionIndex - 1]):
            ...
            previous2 = ""
            # self.list.InsertColumn(0, "Practice",wx.LIST_FORMAT_LEFT, width = 300)
            # self.list.InsertColumn(0, "Problem", wx.LIST_FORMAT_LEFT, width = 300)
            self.list.InsertColumn(0, "Solution", wx.LIST_FORMAT_LEFT, width=600)
            self.list.InsertColumn(1, "In reponsible of", wx.LIST_FORMAT_LEFT, width=300)
            ...
            for index, eachProblemList in enumerate(allTeamProblem):
                ...
                for index2, item in enumerate(eachProblemList):
                    if item[2] == previous1:
                        ...
                        itemIndex = self.list.InsertItem(itemIndex, item[2])

                    self.list.SetItem(itemIndex, 1, item[3])
...
class SparqlQueries:
    ...
    def RunReasoner(self):
        sync_reasoner(self.my_world, debug=1, infer_property_values=True,
                      keep_tmp_file=True)  # reasoner is started and synchronized here
        self.graph = self.my_world.as_rdflib_graph()
        self.result = self.frame_.runQuery.queryAllResult(self.tmpTeam)
    ...
    def search(self, practice, Team=None):
        queryPrefix = u'''PREFIX owl: <http://www.w3.org/2002/07/owl#>
            ...
            '''
        if Team and practice in ["Daily", "Short", "planning", "review", "retrospective"]:
            queryList = [
                '''SELECT DISTINCT   ?practice ?goal
           	 WHERE
            		{
            		?practice rdf:type agile:Practice.
            		?goal rdf:type agile:Goal.
            		?team rdf:type agile:Team.
            		
           	 	?team agile:Achieve ?goal.
           	 	?team agile:Adopt ?practice.
           	 	?practice agile:Achieve ?goal.
           		
            		FILTER(  (regex(str(?practice),  "''' + practice + '''"))&& (regex(str(?team),  "Tmp_team")))
          		
                        }
          		order by ?goal''',

                '''SELECT DISTINCT  ?situation ?requisite
           	 WHERE
            		{
            		?practice rdf:type agile:Practice.
            		?requisite rdf:type agile:Requisite.
            		?team rdf:type agile:Team.
            		?situation rdf:type agile:Situation.
            		?team agile:Have ?situation.
           	 	?team agile:Harm ?requisite.
           	 	?practice agile:Require ?requisite.
           	 ...
           	 WHERE
          		...
          		order by ?situation
                ''',
                '''SELECT DISTINCT  ?team ?problem
           	 WHERE
            		{
            		?practice rdf:type agile:Practice.
            		?problem rdf:type agile:Problem.
            		?team rdf:type agile:Team.
            		
           	 	?practice agile:Encounter ?problem.
           	 	?team agile:Encounter ?problem
            		FILTER(  (regex(str(?practice),  "''' + practice + '''"))&& (regex(str(?team),  "Tmp_team")))
          		
                        }
          		order by ?problem''']

        elif practice in ["Daily", "Short", "planning", "review", "retrospective"]:
    ...
    def queryAllResult(self, Team=None):
        siutationList = []
        ...
        if self.frame_.generalResult == []:
            if os.path.exists(u'queryByPractice.pkl'):
                ...
                for i in range(len(practiceList)):
                    tmp_result.append(self.search(practiceList[i]), Team)
