...
def sync_reasoner_hermit(x=None, infer_property_values=False, debug=1, keep_tmp_file=False):
    ...
    tmp.close()
    command = [owlready2.JAVA_EXE, "-Xmx2000M", "-cp", _HERMIT_CLASSPATH, "org.semanticweb.HermiT.cli.CommandLine",
               "-c", "-O", "-D", "-I", "file:///%s" % tmp.name.replace('\\', '/')]
    ...
    if infer_property_values:
        ...
        for prop_iri, knowns, possibles in _HERMIT_PROP_REGEXP.findall(output):
            ...
            for pair in knowns.split(")("):
                if len(pair[1:-1].split(">, <", 1)) == 1: continue  # sovann
...
def sync_reasoner_pellet(x=None, infer_property_values=False, debug=1, keep_tmp_file=False):
    ...
    if isinstance(x, list):
        ...
        world.save(tmp, format="ntriples")
    tmp.close()
    command = [owlready2.JAVA_EXE, "-Xmx2000M", "-cp", _PELLET_CLASSPATH, "pellet.Pellet", "realize",
               "--ignore-imports", tmp.name]
